...
const sourceDb = options.sourceDb || 'postgresql://postgres:postgres-cbr!000Rr@localhost:5432/afarm_d';
...
async function loadRealCustomers(customers, users) {
  log('🏪 Loading real customers as parties...');
  ...
  for (const customer of customers) {
    if (options.dryRun) {
      log(`Would create party for customer: ${customer.name} (${customer.type})`);
      continue;
    }
    
    try {
      // Create party
      const party = await targetPrisma.parties.create({
        data: {
          displayName: customer.businessName || customer.name,
          legalName: customer.businessName,
          partyType: customer.type === 'B2B' ? 'ORGANIZATION' : 'PERSON',
      ...
      // Create party role
      await targetPrisma.party_roles.create({
        data: {
          partyId: party.id,
          roleType: customer.type === 'B2B' ? 'CUSTOMER_B2B' : 'CUSTOMER_B2C',
      ...
      if (customer.email) {
        await targetPrisma.party_contacts.create({
          data: {
            partyId: party.id,
            type: 'EMAIL',
            label: 'Primary Email',
      ...
      // Also create in customers table for backward compatibility (with partyId link)
      ...
      createdParties.push({ party, customer: createdCustomer });
      log(`✅ Created party & customer: ${party.displayName} (${customer.type})`, 'success');
    } catch (error) {
      log(`❌ Failed to create party for customer ${customer.name}: ${error.message}`, 'error');
    }
  }
